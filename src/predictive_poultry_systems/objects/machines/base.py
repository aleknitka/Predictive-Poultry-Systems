...
class OperationModes(BaseModel):
	...
	off: Annotated[
		float,
		Field(description="consumption, if any per timestep when machine is off."),
	]
...
class BaseMachine(BaseModel):
	...
	outputs: dict[
		MenuItemTypes,
		Annotated[
			float, Field(ge=0.0, description="units produced per timestep when working")
		],
	]
	corporate_name: str
	fault_rate: Annotated[
		float, Field(ge=0.0, le=1.0, description="probability of fault per timestep")
	]
